defmodule ChatAssignmentWeb.UserSessionController do
  ...
  import ChatAssignment.Accounts, only: [get_user_by_login_and_password: 2]
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    case get_user_by_login_and_password(login, password) do
      nil ->
        render_with_error(conn, "Invalid login (username or email) or password")
      ...
      user when user.confirmed_at == nil ->
        render_with_error(
          ...
          [
            "You need to ",
            link("confirm your email", to: Routes.user_confirmation_path(conn, :new)),
            " before you'll be able to login."
          ]
        )
    end
  end
